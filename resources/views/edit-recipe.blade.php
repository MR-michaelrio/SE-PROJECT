...
<div class="container-fluid">
    ...
    @csrf
    <input type="hidden" name="id" value="{{ $recipe[0]->recipe_id }}">
        <div class="row">
            <div class="col-md-8">
                    ...
                    <div class="d-flex flex-column">
                        <div class="p-2">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" value="{{ $recipe[0]->recipe_name }}" name="recipe_name" oninput="updateHeading(this.value)" style="background-color:#F0F2EE; border:1px solid #A6B7BE;" placeholder="Insert Recipe Name">
                            </div>                    
                        </div>
                        <div class="p-2">
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="category_id" style="background-color:#F0F2EE; border:1px solid #A6B7BE;" aria-label="Default select example">
                                    <option value="{{ $recipe[0]->category_id }}" selected>{{ $recipe[0]->Category->category_name }} <- Previews Selected</option>
                                            ...
                                            <tbody id="ingredientsTableBody">
                                                ...
                                                <tr>
                                                    <td class="d-flex align-items-center">
                                                        <input type="text" name="recipe_ingredients[]" value="{{trim($ingredient)}}" placeholder="Ingredients" style="border:0px; border-bottom:1px dashed #A6B7BE; width:100%;outline: none;" oninput="updateIngredientsList()" />
                                                    </td>
                                                    <td>
                                                        <button type="button" name="remove" class="btn btn-danger btn_remove1">X</button>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                    ...
                                    <div class="m-0 p-0" style="width:100%">
                                        <table class="table table-borderless" id="dynamic_field2">
                                            <tbody id="EquipmentTableBody">
                                                @php
                                                    $equipiment = explode(",", $recipe[0]->recipe_equipment);
                                                @endphp
                                                @foreach($equipiment as $equipiments) 
                                                <tr>
                                                    <td class="d-flex align-items-center">
                                                        <input type="text" name="recipe_equipment[]" value="{{ trim($equipiments) }}" placeholder="Equipment" style="border:0px; border-bottom:1px dashed #A6B7BE; width:100%;outline: none;" oninput="updateequipmentList()" />
                                                    </td>
                                                    <td>
                                    ...
                                    <div class="m-0 p-0" style="width:100%">
                                        <table class="table table-borderless" id="dynamic_field3">
                                            <tbody id="StepsTableBody">
                                                @php
                                                    $steps = explode(",", $recipe[0]->recipe_steps);
                                                @endphp
                                                @foreach($steps as $step)
                                                <tr>
                                                    <td class="d-flex align-items-center">
                                                        <input type="text" name="recipe_steps[]" value="{{ trim($step) }}" placeholder="Steps" style="border:0px; border-bottom:1px dashed #A6B7BE; width:100%;outline: none;" oninput="updatestepsList()" />
                                                    </td>
                                                    <td>
                                ...
                                <li class="list-group-item d-flex" style="font-size:1vw; border:0px;">
                                    <div class="m-0 p-0" style="width:100%">
                                        <table class="table table-borderless" id="dynamic_field4">
                                            @php
                                                $tips = explode(",", $recipe[0]->tips);
                                            @endphp
                                            @foreach($tips as $tip)
                                            <tr>
                                                <td class="d-flex align-items-center">
                                                    <input type="text" name="recipe_tips[]" value="{{ trim($tip) }}" placeholder="Tips" style="border:0px; border-bottom:1px dashed #A6B7BE; width:100%;outline: none;" />
                                                </td>
                    ...
                    <div class="d-flex flex-column">
                        <div class="p-2">
                            <div class="card" style="width: 100%;background-color: transparent !important;border:0px !important;">
                                ...
                                <input type="file" name="recipe_picture" id="image" class="form-control" accept=".jpg, .jpeg, .png" style="background-color:#F0F2EE; border:1px solid #A6B7BE;" onchange="previewImage(event)">
                                <img id="preview" src="{{ asset('images/'.$recipe[0]->recipe_picture) }}" style="display:block; width:100%; height:263px; margin-top:20px;">
                                <div class="mt-3 mb-3">
                                    <span><h1 style="font-size:2vw; font-weight:bold" id="recipe-heading">{{ $recipe[0]->recipe_namex }}</h1></span>
...
<script>
    function saveRecipe() {
        var id = '{{ $id }}';
        document.getElementById("recipe-form").action = id;
    }
